...
export const githubPopup = ({ clientId, redirectURI, popup = null }) => {
  ...
  if (popup === null || popup.closed) {
    popup = window.open(
      ...
      // encode url params
      `${GITHUB_URI}?client_id=${clientId}&scope=public_repo&redirect_uri=${redirectURI}`,
 ...
 */
export const getToken = async ({ authURI, code }) => {
  const response = await fetch(`${authURI}/${code}`)
